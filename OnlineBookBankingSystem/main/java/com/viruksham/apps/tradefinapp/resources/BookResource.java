...
import com.viruksham.apps.tradefinapp.model.BookModel;
...
public class BookResource extends BaseResource {
	...
	public String greetUser() {
		return "Thanks for using TradeFinApps Book";
	}

	@GET @Path("/Get/{GetID}")
	@Produces (MediaType.APPLICATION_JSON)
	public Response getUserTypeDataGet(@PathParam("GetID") String GetID) {
System.out.println("entered into Book resource");
		try {
			Object value = getBookModel().getUserTypeGet(GetID);
			if (value == null)
	...
	public Response getUserTypeList() {
		...
		ServiceResponse response = new ServiceResponse();
		try {
			response.setData(getBookModel().getUserTypeList());
			return Response.ok(response.toJsonString()).build();
	...
	public Response getUserTypeDelete(@PathParam("delete") String delete) {
		try {
			Object value = getBookModel().getUserTypeDelete(delete);
			if (value == null)
				//return Response.serverError().build();
				return Response.ok("the book has been deleted").build();
			else {
				if (((List<Object>)value).size() == 0) {
					return Response.noContent().build();
				}
	...
	@Produces (MediaType.APPLICATION_JSON)
	public Response insertdata(String BookJson) {
	 System.out.println("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&");
		...
		System.out.println("entered into insert Book resource");
		try {
			JsonDataObject data = JsonDataObject.valueOf(BookJson);
			response.setData(getBookModel().insertdata(data.getValue()));
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces (MediaType.APPLICATION_JSON)
	public Response editdata(String BookJson) {
		//System.out.println(" Thanks for using TradeFinApps create ");
		System.out.println(" $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ ");
		System.out.println(BookJson);
		response = new ServiceResponse();
		...
		try {
			JsonDataObject data = JsonDataObject.valueOf(BookJson);
			response.setData(getBookModel().editdata(data.getValue()));
			builder.entity(response.toJsonString()).status(Status.OK);
			} catch (Exception e) {
			builder.header("X-Error-Message", e.getMessage());
		}
	...
	private BookModel getBookModel() throws ClassNotFoundException, SQLException, FileNotFoundException {
		if (BookModel == null) {
